...
*Setup instructions for a Gitian build of BHCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BHCoin
...
- [Building BHCoin Core](#building-bhcoin-core)
...
Clone the git repositories for bhcoin and Gitian.
...
in the bhcoin repository under 'Fetch and create inputs' to install sources which require
...
To build BHCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bhcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bhcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bhcoin/bhcoin
...
For example:
```bash
URL=https://github.com/thrasher-/bhcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bhcoin=${COMMIT} --url bhcoin=${URL} ../bhcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bhcoin=${COMMIT} --url bhcoin=${URL} ../bhcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bhcoin=${COMMIT} --url bhcoin=${URL} ../bhcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bhcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bhcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bhcoin
SIGPATH=/some/root/path/bhcoin-detached-sigs

./bin/gbuild --url bhcoin=${BTCPATH},signature=${SIGPATH} ../bhcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bhcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bhcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bhcoin-osx-build.assert
```
...
[bhcoin/gitian.sigs.ltc](https://github.com/bhcoin/gitian.sigs.ltc/) repository, or if that's not possible create a pull
