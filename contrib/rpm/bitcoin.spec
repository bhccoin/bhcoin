...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 16255
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 16256
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 116255
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 116256
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 16255
	%{_sbindir}/semanage port -d -p tcp 16256
	%{_sbindir}/semanage port -d -p tcp 116255
	%{_sbindir}/semanage port -d -p tcp 116256
	for selinuxvariant in %{selinux_variants}; do
